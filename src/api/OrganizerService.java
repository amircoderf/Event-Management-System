...
public class OrganizerService {
    ...
    public static void exportParticipants(int eventId) {
        try {
            String url = BASE_URL+"exportParticipants.php?event_id=" + eventId;
            Desktop.getDesktop().browse(new URI(url));
        } catch (Exception e) {
            e.printStackTrace();
            Alert alert = new Alert(Alert.AlertType.ERROR, "Failed to export participants.");
            alert.showAndWait();
        }
    }
}
